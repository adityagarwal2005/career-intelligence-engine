...
load_dotenv(os.path.join(ROOT_DIR, "optimizer_backend", "backend", ".env"))
...
def require_openai_client():
    if client is None:
        raise RuntimeError(
            "OPENAI_API_KEY is missing. Set it in career_app/.env "
            "or reuse optimizer_backend/backend/.env."
        )
    return client

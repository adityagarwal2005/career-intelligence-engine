...
from services.services.optimizer_service import ResumeOptimizerService
from services.utils.resume_parser import extract_resume_text as backend_extract_resume
...
# Initialize optimizer service once
@st.cache_resource
def get_optimizer_service():
    try:
        settings = get_settings()
        return ResumeOptimizerService(settings)
    except Exception as e:
        st.error(f"Failed to initialize optimizer service: {e}")
        return None
...
with st.sidebar:
    ...
    backend_live = optimizer_service is not None
    ...
    st.markdown(
        ...
        <div class='status-pill'>
            ...
            Optimizer Service {status_label}
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        <div class='side-card'>
            ...
            <p>Optimizes resume content locally for target roles using AI.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with st.expander("Advanced API details"):
        st.markdown("""
Service is now running **locally in Streamlit** — no external server needed!
        """)
...
if selected_section == "Resume Analyzer":
    ...
    optimization_mode = st.radio(
        ...
        horizontal=True,
        help="AI Match = Full freedom | My Dataset = Use your projects",
            ...
            if parse_error:
                ...
                try:
                    ...
                    if not optimizer_service:
                        st.session_state.optimized_resume_error = "Optimizer service failed to initialize."
                    else:
                        with st.spinner(f"Optimizing resume using {optimization_mode}..."):
                            mode_value = "ai_match" if "AI Match" in optimization_mode else "my_dataset"
                            optimized_result = optimizer_service.optimize(
                        ...
                        if optimized_result:
                            ...
                            st.session_state.optimized_resume_error = "Optimizer returned an empty result."

                except Exception as exc:
                    st.session_state.optimized_resume_error = f"Optimization failed: {exc}"
